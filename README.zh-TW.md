...
這樣可以讓倉庫名更偏功能與開源語境，同時保留 SuperNoNo 作為產品代號。更多命名方案見 [REPOSITORY_NAMING.md](REPOSITORY_NAMING.md)。
...
├── REPOSITORY_NAMING.md   倉庫命名與開源發布建議
...
開源版本應該只包含你擁有權利或明確可公開分發的素材。

如果本地使用了克隆專案、第三方圖片、Live2D 模型、貼圖、動作檔或遊戲 IP 相關素材，請不要上傳到 GitHub，除非這些素材的授權明確允許你這樣做。

需要特別注意：

- 不上傳 `C:\Users\66460\Desktop\SuperNoNo\SuperNoNo` 這個克隆目錄是必要的。
- 如果你已經把該目錄裡的素材複製到本專案其他目錄，也需要在開源前刪除或排除這些複製品。
- README 不應宣稱第三方素材是原創。
- 建議公開版本使用原創 SVG、程式化生成圖示或重新設計的原創角色形象。

這不是正式法律意見。如果計畫公開傳播、商業使用或長期維護，建議完成一次版權和商標檢查。
...
注意：MIT License 只適用於本倉庫中你有權開源的程式碼和原創素材，不會自動授權第三方素材、遊戲 IP、Live2D 模型或外部專案資源。
